...
const createCall = async (req, res) => {
  try {
    ...
    fs.createReadStream(csvFilePath).pipe(csv()).on('data', (row) => { result.push(row) }).on('end', async () => {
      console.log("CSV loaded: ", result)

      for (const leads of result) {
        ...
        try {
          const response = await axios.post(
            "https://api.vapi.ai/call",
            {
              ...
              assistantOverrides: {
                firstMessage: `Hi, is this ${leads.name}? ${leads.name}, this is Luna from Liveforce.`,
                voicemailDetection: {
                  provider: "twilio",
                  enabled: false
                },
                ...
                variableValues: {
                  ...
                  industrial_detail: leads.industrial_detail,
                  linkedin: leads.linkedin,
                  industry: leads.industry,
                  number_of_employees: leads.number_of_employees,
                },
          ...
          console.log(`Call initiated for ${leads.name}:`, response.data);



        }
        catch (error) {
          console.error(`Error in calling ${leads.name}:`, error.response?.data || error.message)
        }
      }
    })
    res.status(200).json({
      message: "Calls are started.",
      total: result.length,
    })
...
const ListCall = async (req, res) => {
  try {
    ...
    if (response.data) {
      console.log("list of calls found")
      return res.json({
        success: true,
        data: response.data
      }).status(200)
...
export { createCall, ListCall, CallById } 
